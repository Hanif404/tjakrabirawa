<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Work extends CI_Controller
    {
	    ...
	    $config['base_url'] = base_url().'admin/list-work/page/';
	    ...
	    $this->pagination->initialize($config);
            ...
            $this->data['berita'] = $this->admin_m->detail_work($config['per_page'], $offset);

            $this->data['work_active_treeview'] = "active";
            $this->data['work_active_list'] = "active";
            $this->data['title'] = "Administrator - List Work";
            $this->load->view('element/admin',$this->data);
            $this->load->view('pages/admin/work');
        ...
        public function add_work()
        {
            ...
            $this->data['work_active_treeview'] = "active";
            $this->data['work_active_add'] = "active";
            $this->data['title'] = "Administrator - Add Work";
            $this->load->view('element/admin',$this->data);
            $this->load->view('pages/admin/add-work');
            $this->load->view('element/footer-admin');
        }
            ...
            if($_FILES['userfile']['name'])
            {
                if ($this->upload->do_upload('userfile'))
                {
                    ...
                    $data = array(
                        ...
                        'kategori'          => 'work', //$this->input->post('kategori', TRUE),
                    ...
                    $this->session->set_flashdata("pesan", "<div class=\"alert alert-success\" id=\"alert\">Work Berhasil Di Tambahkan !!</div>");
                    redirect('admin/list-work');
                }else{
                    $this->session->set_flashdata("pesan", "<div class=\"alert alert-danger\" id=\"alert\">Terjadi Kesalahan, Gagal Menyimpan Work !!<br/>".$this->upload->display_errors()."</div>");
                    redirect('admin/add-work');
                }
            }else{
        ...
        public function edit_work($id)
        {
            if($this->session->userdata('SESS_ADMIN') != TRUE){ redirect('admin/login-page'); }
            $id_admin = $this->session->userdata('ADMIN_ID');
            if($id == NULL){ redirect('admin/list-work');}
            ...
            $this->data['work_active_treeview'] = "active";
            $this->data['work_active_add'] = "active";
            $this->data['title'] = "Administrator - Edit Work";
            $this->load->view('element/admin',$this->data);
            $this->load->view('pages/admin/edit-work');
            $this->load->view('element/footer-admin');
        }
        ...
        public function update_work()
        {
            ...
            if($_FILES['userfile']['name'])
            {
                if ($this->upload->do_upload('userfile'))
                {
                    ...
                    $this->session->set_flashdata("pesan", "<div class=\"alert alert-danger\" id=\"alert\">Terjadi Kesalahan, Gagal Menyimpan Work !!</div>");
                    redirect('admin/list-work');
                }
            }else{
					         $id_berita = $this->input->post('id', TRUE);
